...
uint16_t UpdateCrcKermit(uint16_t Init_Value, uint8_t value)
{
  ...
  short_c = 0x00ff & (uint16_t) value;
  tmp =  Init_Value ^ short_c;
  Init_Value = (Init_Value >> 8) ^ KermitTable[tmp & 0xff];
...
/*****CRC calculation for 16bit data********/
#pragma CODE_SECTION(_crc16,".TI.ramfunc");
...
#pragma CODE_SECTION(CalcCRC_Bloc,".TI.ramfunc");
uint16_t CalcCRC_Bloc(uint32_t Address, uint16_t LenDataToCopy, uint8_t WriteMemoryArea, uint8_t* DataToCopy)
{
    int k;
    uint16_t CRCCalc = 0;
    /*Crc on Memory Area*/
    CRCCalc = UpdateCrcKermit(CRCCalc, WriteMemoryArea);
    /*Crc on Address*/
    CRCCalc = UpdateCrcKermit(CRCCalc, (uint8_t) (Address >> 16));
    CRCCalc = UpdateCrcKermit(CRCCalc, (uint8_t) (Address >> 8));
    CRCCalc = UpdateCrcKermit(CRCCalc, (uint8_t) Address);
    /*Crc on Memory Size*/
    ...
    for (k = 0; k < LenDataToCopy; k++)
    {
        ...
        CRCCalc = UpdateCrcKermit(CRCCalc, DataToCopy[k]);
    }
...
/******************************************************************************
 *    Function:          CalcCRC_FLASH
 *    Description:       Calculate CRC Kermit on Appli Area
 *    Return Value:      Calculated CRC16
 ******************************************************************************/
